...
# This script is a follow up to clone7.sh.
...
echo "Updating system...  This is unnecessary, but it gives the user extra time"
echo "to shutdown if there is a problem."
...
echo "Generating script to copy sdb and sdc to sde, sdf, sdi, sdj, sdm, and"
echo "sdn..."
...
echo "echo Machine 1 is done. >> /tmp/report.log" >> /tmp/ddroutine.sh
echo "echo sde and sdf can be removed. >> /tmp/report.log" >> /tmp/ddroutine.sh
...
echo "echo Machine 3 is done. >> /tmp/report.log" >> /tmp/ddroutine.sh
echo "echo sdi and sdj can be removed. >> /tmp/report.log" >> /tmp/ddroutine.sh
echo "uptime >> /tmp/report.log" >> /tmp/ddroutine.sh
# 2:30AM
...
echo "echo Machine 3 is done. >> /tmp/report.log" >> /tmp/ddroutine.sh
echo "echo sdm and sdn can be removed. >> /tmp/report.log" >> /tmp/ddroutine.sh
echo "uptime >> /tmp/report.log" >> /tmp/ddroutine.sh
# 8:00AM

echo "Copying sdb and sdc to sde, sdf, sdi, sdj, sdm, and sdn in the"
echo "background..."
...
# 12 hours of extra time for the other script to complete.
echo "sleep 43200" >> /tmp/shutdown.sh
