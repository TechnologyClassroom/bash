# updateallhosts.sh
# Michael McMahon
# This will take updates from HOSTSMTWRwin.txt and the top portion of
# HOSTSFwin.txt to create a new set of HOSTS files.
...
# Syntax from http://unix.stackexchange.com/questions/181283

# Cut everthing from '# Normal Hosts' to the end of the file and save that as
# normalhosts.txt (has Windows end-of-line format)
echo "Creating temporary files..."
grep -n '# Normal Hosts' HOSTSMTWRwin.txt | cut -f1 -d":" | xargs -I{} tail -n +{} HOSTSMTWRwin.txt > normalhosts.txt
# Syntax from http://stackoverflow.com/questions/5968107
# grep -n FOO FILE.TXT | cut -f1 -d":" | xargs -I{} tail -n +{} FILE.TXT > NEWFILE.TXT

# Replace everything at '# Normal Hosts' with normalhosts.txt for a file
# containing 'F'
...
# syntax from http://unix.stackexchange.com/questions/112023
